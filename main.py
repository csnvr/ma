...
class ScreenWelcome(Screen):
    ...
    def welcome_button(self):
        global nickname
        nickname=self.ids.phone_input.text

        global st_invalid_pn # skrbi da ni na info labelu vec kot 3 napisi oz pac steje invalid napise
        if len(nickname) > 2:
            valid=True
            #print(phoneN)
        else:
...
class ScreenMain(Screen):
    ...
    def start_multiplayer(self):
        sound = SoundLoader.load('other_files/bloody.mp3')
        if sound:
            print('FU')
        #sound.play()
...
# ce je konec igre zakljuci igro
        if end_type == 'X' and not popup:
            ...
            self.dialog = MDDialog(title=f'Congrats!' if gamemode == 'coop' else f'Congrats {nickname}!', text=f'X has {score_x} wins\nand\nO has {score_o} wins' if gamemode == 'coop' else f"You've won!", size_hint=(0.5, 0.99),
            ...
            score_o += 1 # pristeje score o
            popup =True
            ok_button = MDFlatButton(text='OK', on_release=self.ok_button)
            playagain_button = MDFlatButton(text='Play Again', on_release=self.playagain_button)
            self.dialog = MDDialog(title='Congrats O!' if gamemode == 'coop' else f'You Lost! :(', text=f'X has {score_x} wins\nand\nO has {score_o} wins' if gamemode == 'coop' else f'Oh {nickname}, better luck next time!', size_hint=(0.5, 0.99),
                                   buttons=[playagain_button, ok_button])
            self.dialog.open()
            #time.sleep(2)
            #self.dialog.dismiss()
            ...
            ok_button = MDFlatButton(text='OK', on_release=self.ok_button)
            playagain_button = MDFlatButton(text='Play Again', on_release=self.playagain_button)
            self.dialog = MDDialog(title="It's a Tie!", text='Better luck next time!' if gamemode == 'coop' else f'Better luck next time {nickname}!', size_hint=(0.5,0.99),
                                   buttons=[playagain_button, ok_button])
            self.dialog.open()
            #print('izenaceno')
            #time.sleep(2)
...
# da najprej pregleda če lahko zmaga in šele potem zapira igralca
        for ox in ['O', 'X']:
    # preverja za vse možne vodoravne
            if not already:
                for x in [1, 4, 7]:
                    if Gameboard[x] == Gameboard[x + 1] and Gameboard[x + 2] not in ['X', 'O'] and Gameboard[x] == ox:
                        Gameboard[x + 2] = znak
                        already = True
                        break
                    elif Gameboard[x] == Gameboard[x + 2] and Gameboard[x + 1] not in ['X', 'O'] and Gameboard[x] == ox:
                        Gameboard[x + 1] = znak
                        already = True
                        break
                    elif Gameboard[x + 1] == Gameboard[x + 2] and Gameboard[x] not in ['X', 'O'] and Gameboard[x+1] == ox:
                        Gameboard[x] = znak
                        already = True
                        break
    ...
    # preverja za vse možne navpične
            if not already:
                for x in [1, 2, 3]:
                    if Gameboard[x] == Gameboard[x + 3] and Gameboard[x + 6] not in ['X', 'O'] and Gameboard[x] == ox:
                        Gameboard[x + 6] = znak
                        already = True
                        break
                    elif Gameboard[x] == Gameboard[x + 6] and Gameboard[x + 3] not in ['X', 'O'] and Gameboard[x] == ox:
                        Gameboard[x + 3] = znak
                        already = True
                        break
                    elif Gameboard[x + 3] == Gameboard[x + 6] and Gameboard[x] not in ['X', 'O'] and Gameboard[x+3] == ox:
                        Gameboard[x] = znak
                        already = True
                        break
    ...
    # preverja za 1. diagonalo
            if not already:
                if Gameboard[1] == Gameboard[5] and Gameboard[9] not in ['X', 'O'] and Gameboard[1] == ox:
                    Gameboard[9] = znak
                    already = True
                elif Gameboard[1] == Gameboard[9] and Gameboard[5] not in ['X', 'O'] and Gameboard[1] == ox:
                    Gameboard[5] = znak
                    already = True
                elif Gameboard[5] == Gameboard[9] and Gameboard[1] not in ['X', 'O'] and Gameboard[5] == ox:
                    Gameboard[1] = znak
                    already = True

    # preverja za 2. diagonalo
            if not already:
                if Gameboard[3] == Gameboard[5] and Gameboard[7] != ox and Gameboard[3] == ox:
                    Gameboard[7] = znak
                    already = True
                elif Gameboard[3] == Gameboard[7] and Gameboard[5] != ox and Gameboard[3] == ox:
                    Gameboard[5] = znak
                    already = True
                elif Gameboard[5] == Gameboard[7] and Gameboard[3] != ox and Gameboard[5] == ox:
                    Gameboard[3] = znak
                    already = True
